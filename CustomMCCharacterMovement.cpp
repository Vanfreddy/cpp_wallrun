...
//----------------------------------------------------------------------//
UCustomMCCharacterMovement::UCustomMCCharacterMovement(const FObjectInitializer& ObjectInitializer)
...
FNetworkPredictionData_Client* UCustomMCCharacterMovement::GetPredictionData_Client() const
{
	if (ClientPredictionData == nullptr)
	{
		// Return our custom client prediction class instead
		UCustomMCCharacterMovement* MutableThis = const_cast<UCustomMCCharacterMovement*>(this);
		MutableThis->ClientPredictionData = new FNetworkPredictionData_Client_CustomMCCharacter(*this);
	}

	return ClientPredictionData;
}

void UCustomMCCharacterMovement::BeginPlay()
{
	Super::BeginPlay();
}

void UCustomMCCharacterMovement::TickComponent(float DeltaTime, enum ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
void UCustomMCCharacterMovement::MoveAutonomous(float ClientTimeStamp, float DeltaTime,
	uint8 CompressedFlags, const FVector& NewAccel)
{
	FCustomMCCharacterNetworkMoveData* Move = static_cast<FCustomMCCharacterNetworkMoveData*>(GetCurrentNetworkMoveData());
	if (Move != nullptr)
	{
...
void UCustomMCCharacterMovement::UpdateCharacterStateBeforeMovement(float DeltaSeconds)
{
...
void UCustomMCCharacterMovement::UpdateCharacterStateAfterMovement(float DeltaSeconds)
{
	Super::UpdateCharacterStateAfterMovement(DeltaSeconds);
}

bool UCustomMCCharacterMovement::DoJump(bool bReplayingMoves)
{
...
void UCustomMCCharacterMovement::OnMovementUpdated(float DeltaSeconds, const FVector& OldLocation, const FVector& OldVelocity)
{
...
void UCustomMCCharacterMovement::OnMovementModeChanged(EMovementMode PreviousMovementMode, uint8 PreviousCustomMode)
{
	Super::OnMovementModeChanged(PreviousMovementMode, PreviousCustomMode);
}


void UCustomMCCharacterMovement::PhysCustom(float deltaTime, int32 Iterations)
{
...
float UCustomMCCharacterMovement::GetMaxSpeed() const
{
	switch (MovementMode)
	{
	case MOVE_Walking:
...
float UCustomMCCharacterMovement::GetMaxAcceleration() const
{
...
float UCustomMCCharacterMovement::GetMaxBrakingDeceleration() const
{
	if (MovementMode == MOVE_Custom)
	{
		switch (CustomMovementMode)
...
void UCustomMCCharacterMovement::ProcessLanded(const FHitResult& Hit, float remainingTime, int32 Iterations)
{
...
void UCustomMCCharacterMovement::UpdateFromCompressedFlags(uint8 Flags)
{
...
void UCustomMCCharacterMovement::DoWallRunJump(bool bReplayingMoves)
{
...
void UCustomMCCharacterMovement::StartWallRunning(EWallRunSide Side, FVector InWallNormal, FVector InWallRunTraceImpactPoint)
{
	// Clean up
...
void UCustomMCCharacterMovement::StopWallRunning()
{
	StartWallRunCooldown(WallRunSide);
	SetMovementMode(EMovementMode::MOVE_Falling);
}

bool UCustomMCCharacterMovement::CanStartWallRunSide(EWallRunSide Side, FVector& OutWallNormal, FVector& OutImpactPoint)
{
...
bool UCustomMCCharacterMovement::TraceNearbyForWalls(EWallRunSide Side, bool bFallbackToFeetLevel, FVector& OutNormal, FVector& OutImpactPoint)
{
...
void UCustomMCCharacterMovement::UnstickFromWallPressed()
{
...
void UCustomMCCharacterMovement::UnstickFromWall_Internal()
{
	if (IsWallRunning())
	{
...
void UCustomMCCharacterMovement::ResetUnstickFromWall()
{
	WantsToUnstickTimeRemaining = UnstickFromWallTimeThreshold;
	bWallrunWantsToUnstick = false;
}

float UCustomMCCharacterMovement::GetWallRunGravityScale()
{
...
bool UCustomMCCharacterMovement::IsWallRunning()
{
	return MovementMode == MOVE_Custom && CustomMovementMode == ECustomMovementMode::CMOVE_WallRunning && UpdatedComponent;
}

bool UCustomMCCharacterMovement::IsWallRunningSide(EWallRunSide Side)
{
	return IsWallRunning() && WallRunSide == Side;
}

void UCustomMCCharacterMovement::StartWallRunCooldown(EWallRunSide Side) 
{
...
bool UCustomMCCharacterMovement::IsWallRunOnCooldown(EWallRunSide Side) 
{
	if (Side == EWallRunSide::Left) {
...
void UCustomMCCharacterMovement::OnComponentDestroyed(bool bDestroyingHierarchy)
{
	Super::OnComponentDestroyed(bDestroyingHierarchy);
}

float UCustomMCCharacterMovement::GetCurrentCameraTilt()
{
	return CurrentCameraTiltAlpha* CameraMaxTilt * -1.0f;
}

FVector UCustomMCCharacterMovement::GetWallRunForwardDirection()
{
...
FVector UCustomMCCharacterMovement::GetWallRunForwardDirection(EWallRunSide Side, FVector WallNormal)
{
...
FVector UCustomMCCharacterMovement::GetWallRunLateralAcceleration(float deltaTime)
{
	return FVector(Acceleration.X, Acceleration.Y, 0.f);
}

void UCustomMCCharacterMovement::TransitionWallRunToEndState()
{
...
void UCustomMCCharacterMovement::PhysWallRunning(float deltaTime, int32 Iterations)
{
